...
Во первых, давайте для удобства начала скажем, что вертикальные отрезки не имеют смысла(так как если упасть на начало отрезка, то докатившись до конца, мы останемся в той же $x$ координате), поэтому их можно удалить, очевидно это делается за $O(n)$.

Введем событие обновления - отрезок с координатами $(x_1, y_1, x_2, y_2)$, где $y_1 \ge y_2$.

Давайте отсортируем все события по $y$ координате(сортируем по $y_2$, то есть по "нижнему" концу). Это будет работать за $O(n \log n)$.

Воспользуемся сканирующей прямой параллельно $Оx$ и будем обрабатывать события по мере их наступления от "нижних" до "высоких"(то есть по возрастанию $y$).

Заведем стандартное дерево Ли-Чао на $[min(x_i), max(x_j)]$, то есть минимальный $x$ среди отрезков и макс. $x$ среди всех отрезков. Дерево будет отвечать уметь добавлять кусочно-заданные функции, а также находить пару {макс. значение, на каком отрезке достигается это макс. значение}. А также для каждого отрезка будем хранить ответ для данного отрезка.

#### Обработка событий

Будем добавлять отрезки по списку сортировки событий. 
Пусть текущий отрезок $(x_1, y_1, x_2, y_2)$, где $y_1 \ge y_2$, тогда вычислим для $x_1$ {макс. значение, на каком отрезке это значение}, и скажем, что это ответ для текущего отрезка это номер отрезка, где нашелся этот макс. Заметим, что возможно мы нашли либо первый отрезок ниже чем

$$
f(x) = y_1 + \frac{y_2-y_1}{x_2-x_1}(x - x_1), \text {на отрезке $[\min(x_1,x_2),\;\max(x_1,x_2)].$}
$$

Заметим, что добавив этот отрезок мы актуальную 
